...
pub extern "system" fn Java_com_lancedb_lance_file_LanceFileReader_readAllNative(
    ...
    let reader = unsafe { env.take_rust_field::<_, _, BlockingFileReader>(reader, NATIVE_READER) }.unwrap();
    ...
    if !projected_names.is_null() {
        ...
        reader_projection = Some(ReaderProjection::from_column_names(&schema, names.as_slice()).unwrap());
    }
            ...
            if (array_length > 0) {
                ...
                for i in (0..array_length as usize).step_by(2) {
                    ...
                    if start_i32 < 0 || end_i32 < 0 {
                        env.throw_new("java/lang/IllegalArgumentException", "Range values must be non-negative")?;
                    }
                    if start_i32 > end_i32 {
                        env.throw_new("java/lang/IllegalArgumentException", "Range start must be less than or equal to end")?;
                    }
